...
from .const import MEDIA_FILTER_OPTIONS
...
async def async_setup_entry(hass, entry: MusicConfigEntry, async_add_entities) -> None:
    ...
    async_add_entities(
        [
            ...
            MediaFilterSelect(controller),
            MediaSelect(controller),
...
class MediaFilterSelect(MusicRestoreEntity, SelectEntity):
    """Filter the combined media dropdown to playlists, stations, or all."""

    _attr_translation_key = "media_filter"
    _attr_options = list(MEDIA_FILTER_OPTIONS)

    def __init__(self, controller: MusicController) -> None:
        """Initialize the media filter select."""

        super().__init__(controller, "media_filter")

    @property
    def current_option(self) -> str | None:
        """Return the active filter."""

        return self._controller.selected_media_filter

    async def async_added_to_hass(self) -> None:
        """Restore the previous filter and track controller changes."""

        await super().async_added_to_hass()
        previous = await self.async_get_last_state()
        if previous and previous.state in self.options:
            self._controller.set_selected_media_filter(previous.state)
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        self._controller.set_selected_media_filter(option)


class MediaSelect(MusicRestoreEntity, SelectEntity):
    """Select a radio station or playlist from the combined catalog."""

    _attr_translation_key = "media"

    def __init__(self, controller: MusicController) -> None:
        """Initialize the combined media select."""

        super().__init__(controller, "media")

    @property
    def options(self) -> list[str]:
        """Return the catalog labels matching the active filter."""

        return self._controller.media_options()

    @property
    def current_option(self) -> str | None:
        """Return the selected media label."""

        return self._controller.selected_media

    @property
    def available(self) -> bool:
        """Return whether any media is configured under the active filter."""

        return bool(self.options)

    async def async_added_to_hass(self) -> None:
        """Restore the previous selection and track controller changes."""

        await super().async_added_to_hass()
        previous = await self.async_get_last_state()
        if previous and previous.state in self.options:
            self._controller.set_selected_media(previous.state)
        self.async_on_remove(
            self._controller.async_add_listener(self.async_write_ha_state)
        )

    async def async_select_option(self, option: str) -> None:
        """Select a media label."""

        self._controller.set_selected_media(option)
